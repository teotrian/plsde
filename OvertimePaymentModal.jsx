...
const OvertimePaymentModal = ({ active, handleCreateModal, token, id, setErrorMessage }) => {
  const [name, setName] = useState("");       //Variable for the selected school
  const [month, setMonth] = useState("");
  const [year, setYear] = useState("");
  
  useEffect(() => {
    const getOvertimePayment = async () => {
      ...
      if (!response.ok) {
        setErrorMessage("Κάτι πήγε στραβά. Δεν μπόρεσε να φορτωθεί η κατάσταση πληρωμής υπερωριών");
      } else {
  ...
  const handleCreateOvertimePayment = async (e) => {
    e.preventDefault();
    ...
    const response = await fetch("/api/admin/overtimepayment", requestOptions);
    if (!response.ok) {
      setErrorMessage("Κάτι πήγε στραβά. Δεν μπόρεσε να δημιουργηθεί η κατάσταση πληρωμής υπερωριών");
    } else {
      cleanFormData();
      handleCreateModal();
    }
  ...
  const handleUpdateOvertimePayment = async (e) => {
    e.preventDefault(); 
    const requestOptions = {
      method: "PUT",
      headers: {
    ...
    const response = await fetch(`/api/admin/overtimepayment/${id}`, requestOptions);
    if (!response.ok) {
      setErrorMessage("Κάτι πήγε στραβά. Δεν μπόρεσε να ενημερωθεί η κατάσταση πληρωμής υπερωριών");	
    } else {
      cleanFormData();
      handleCreateModal();
    }
      ...
      <div className="modal-card">
        <header className="modal-card-head has-background-warning-light">
          <h1 className="modal-card-title">
            {id ? "Επεξεργασία Κατάστασης Πληρωμής Υπερωριών" : "Δημιουργία Κατάστασης Πληρωμής Υπερωριών"}
          </h1>
        </header>
        ...
        <footer className="modal-card-foot has-background-warning-light">
          {id ? (
            <button className="button is-info" onClick={handleUpdateOvertimePayment}>
              Αποθήκευση
            </button>
          ) : (
            <button className="button is-primary" onClick={handleCreateOvertimePayment}>
              Δημιουργία
            </button>
          )}
...
export default OvertimePaymentModal;
